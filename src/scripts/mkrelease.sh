...
sed -i "s/${PREV_VERSION}/${APP_VERSION}/g" zushdragon.pro > /dev/null
...
$QT_STATIC/bin/qmake zushdragon.pro -spec linux-clang CONFIG+=release > /dev/null
echo "[OK]"
...
cat zushdragon.pro | sed "s/precompile_header/release/g" | sed "s/PRECOMPILED_HEADER.*//g" > zec-qt-wallet-mingw.pro
echo "[OK]"
