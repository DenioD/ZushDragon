                ...
                if (tryEzcashdStart) {
                    ...
                    if (config->zcashDaemon) {
                        explanation = QString() % QObject::tr("You have hushd set to start as a daemon, which can cause problems "
                            "with ZushDragon\n\n."
                            "Please remove the following line from your HUSH3.conf and restart ZushDragon\n"
                            "daemon=1");
                    } else {
...
void ConnectionLoader::createZcashConf() {
    ...
    out << "# Autogenerated by Hush ZushDragon https://myhush.org\n";
    out << "server=1\n";
    out << "rpcuser=hush\n";
...
#ifdef Q_OS_LINUX
    auto confLocation = QStandardPaths::locate(QStandardPaths::HomeLocation, ".komodo/ZUSH/ZUSH.conf");
#elif defined(Q_OS_DARWIN)
    auto confLocation = QStandardPaths::locate(QStandardPaths::HomeLocation, "Library/Application Support/Komodo/ZUSH/ZUSH.conf");
#else
    auto confLocation = QStandardPaths::locate(QStandardPaths::AppDataLocation, "../../Komodo/ZUSH/ZUSH.conf");
#endif
...
#ifdef Q_OS_LINUX
    auto confLocation = QDir(QStandardPaths::writableLocation(QStandardPaths::HomeLocation)).filePath(".komodo/ZUSH/ZUSH.conf");
#elif defined(Q_OS_DARWIN)
    auto confLocation = QDir(QStandardPaths::writableLocation(QStandardPaths::HomeLocation)).filePath("Library/Application Support/Komodo/ZUSH/ZUSH.conf");
#else
    auto confLocation = QDir(QStandardPaths::writableLocation(QStandardPaths::AppDataLocation)).filePath("../../Komodo/ZUSH/ZUSH.conf");
#endif
...
bool ConnectionLoader::verifyParams() {
    ...
    if( QFile( QDir("/Applications").filePath("zushdragon.app/Contents/MacOS/sapling-output.params") ).exists() && QFile( QDir("/Applications").filePath("./zushdragon.app/Contents/MacOS/sapling-spend.params") ).exists() ) {
        qDebug() << "Found params in /Applications/zushdragon.app/Contents/MacOS";
        return true;
    }

    // this is to support SD on mac inside a DMG
    if( QFile( QDir("./").filePath("zushdragon.app/Contents/MacOS/sapling-output.params") ).exists() && QFile( QDir("./").filePath("./zushdragon.app/Contents/MacOS/sapling-spend.params") ).exists() ) {
        qDebug() << "Found params in ./zushdragon.app/Contents/MacOS";
        return true;
    }
...
std::shared_ptr<ConnectionConfig> ConnectionLoader::autoDetectZcashConf() {    
    ...
    if (zcashconf->port.isEmpty()) zcashconf->port = "23136";
    file.close();
